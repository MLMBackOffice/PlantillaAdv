 ...
 */
class CincoSearch extends Users
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        $query= (new \yii\db\Query())
                ->select(['(case when u5.estado=1 then "Registrados" else case when u5.estado=2 then "Activos" else "Inactivos" end end) as Estado',
                        'count(DISTINCT u5.id) as Cantidad'])
                   // 'u.estado as Estado'])
                ->from('users p')
                ...
                ->andWhere(['<>', 'u5.estado', 0])
                ->groupBy('u1.patrocinador, u5.estado');
