...
class DosSearch extends Users
{
    ...
    public function search($params)
    {
        $query= (new \yii\db\Query())
                ->select(['(case when u2.estado=1 then "Registrados" else case when u2.estado=2 then "Activos" else "Inactivos" end end) as Estado',
                        'count(DISTINCT u2.id) as Cantidad'])
                   // 'u.estado as Estado'])
                ->from('users p')
