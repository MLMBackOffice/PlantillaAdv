<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message/extract' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    'About' => '',
    'Home' => 'Inicio',
    'Status' => '',
    
    // models
    'ID' => 'Código',
    'User ID' => 'Código de Usuario',
    'Created At' => 'Fecha de Creación',
    'Updated At' => 'Fecha de Actualización',
    'Full Name' => 'Nombre completo',
    'Username' => 'Nombre de Usuario',

    'Name' => 'Nombre',
    'Can Admin' => 'Puede Administrar',

    'Role' => 'Rol',
    'Role ID' => 'Código de Rol',
    'Status' => 'Estado',
    'Email' => 'Correo electrónico',
    'New Email' => 'Nuevo correo electrónico',
    'Username' => 'Nombre de Usuario',
    'Password' => 'Contraseña',
    'Password Repeat' => 'Repetir Contraseña',
    'Auth Key' => 'Clave de Autorización',
    'Access Token' => 'Token de Acceso',
    'Logged In Ip' => 'IP de Conexión',
    'Logged In At' => 'Fecha de Conexión',
    'Created Ip' => 'IP de Creación',
    'Banned At' => 'Tiempo de Expulsión',
    'Banned Reason' => 'Razón de Expulsión',
    'Current Password' => 'Contraseña Actual',
    'New Password' => 'Nueva Contraseña',
    'New Password Confirm' => 'Confirmación de nueva contraseña',
    'Email Confirmation' => 'Confirmación de correo',
    
    'Provider' => 'Proveedor',
    'Provider ID' => 'Código de proveedor',
    'Provider Attributes' => 'Atributos de proveedor',
    

    'Type' => 'Tipo',
    'Token' => 'Token',
    'Expired At' => 'Fecha de Expiración',

    // models/forms
    'Email not found' => 'Correo no encontrado',
    'Email / Username' => 'Correo / Usuario',
    'Email / Username not found' => 'Correo / Usuario no encontrado',
    'Username not found' => 'Usuario no encontrado',
    'User is banned - {banReason}' => 'El usuario está expulsado - {banReason}',
    'Incorrect password' => 'Contraseña incorrecta',
    'Remember Me' => 'Recuérdame',
    'Email is already active' => 'El correo ya está activo',
    'Passwords do not match' => 'Las contraseñas no coinciden',
    '{attribute} can contain only letters, numbers, and "_"' => '{attribute} puede contener tan solo letras, números y "_"',

    // controllers
    'Successfully registered [ {displayName} ]' => 'Registrado con éxito [ {displayName} ]',
    ' - Please check your email to confirm your account' => ' - Por favor, comprueba tu correo para confirmar el registro',
    'Account updated' => 'Cuenta modificada',
    'Profile updated' => 'Perfil modificado',
    'Confirmation email resent' => 'Correo de confirmación reenviado',
    'Email change cancelled' => 'Cambio de correo cancelado',
    'Instructions to reset your password have been sent' => 'Las instrucciones para reiniciar tu contraseña han sido enviadas',

    // mail
    'Please confirm your email address by clicking the link below:' => 'Por favor, confirma tu dirección de correo haciendo click en el enlace de abajo:',
    'Please use this link to reset your password:' => 'Por favor, usa este link para reiniciar tu contraseña:',

    // admin views
    'Users' => 'Usuarios',
    'Banned' => 'Expulsado',
    'Create' => 'Crear',
    'Update' => 'Actualizar',
    'Delete' => 'Eliminar',
    'Search' => 'Búsqueda',
    'Reset' => 'Reiniciar',
    'Create {modelClass}' => 'Crear {modelClass}',
    'Update {modelClass}: ' => 'Actualizar {modelClass}: ',
    'Are you sure you want to delete this item?' => '¿Está seguro de que desea eliminar este elemento?',

    // default views
    'Account' => 'Cuenta',
    'Pending email confirmation: [ {newEmail} ]' => 'Confirmación de correo pendiente: [ {newEmail} ]',
    'Cancel' => 'Cancelar',
    'Changing your email requires email confirmation' => 'Cambiar tu dirección de correo requiere confirmación',
    'Confirmed' => 'Confirmado',
    'Error' => 'Error',
    'Your email [ {email} ] has been confirmed' => 'Tu dirección de correo [ {email} ] ha sido confirmada',
    'Go to my account' => 'Ir a mi cuenta',
    'Go home' => 'Inicio',
    'Log in here' => 'Iniciar sesión',
    'Invalid Token' => 'Token inválida',
    'Forgot password' => 'Contraseña olvidada',
    'Submit' => 'Enviar',
    'Yii 2 User' => 'Usuario de Yii 2',
    'Login' => 'Iniciar Sesión',
    'Register' => 'Registro',
    'Logout' => 'Salir',
    'Resend confirmation email' => 'Reenviar correo de confirmación',
    'Resend' => 'Reenviar',
    'Password has been reset' => 'La contraseña ha sido reiniciada',
//    'Login link sent - Please check your email' => '',
//    'Registration link sent - Please check your email' => '',
    
    //Nuevos
    'Wallet Address' => 'Dirección Billetera',
    'Country' => 'País',
    'Sponsor' => 'Patrocinador',
    'Birthdate' => 'Fecha de Nacimiento',
    'Language' => 'Idioma',
    'Affiliate Network' => 'Red de Afiliados',
    'Memberships' => 'Membresías',
    'Commissions' => 'Comisiones',
    'Reports' => 'Reportes',
    'MAIN NAVIGATION' => 'MENÚ DE NAVEGACIÓN',
    'Please fill out the following fields to signup' => 'Por favor rellena los siguientes campos para registrarte',
    'If you forgot your password you can' => 'Si olvidaste tu contraseña puedes',
    'Restore Password' => 'Reestablecer Contraseña',
    'Update Your Information' => 'Actualiza tus Datos',
    'Join WeiFastPay' => 'UNIRSE A WEIFASTPAY',
    'Thank you for your desire to enter WeiFastPay. Soon you will know, it will be one of the best decisions of your life!' => 'Gracias por tu deseo de ingresar a WeiFastPay. Pronto sabrás, que será una de las mejores decisiones de su vida!',
    'Profile'=>'Perfil',
    'Dashboard' => 'Panel',
    'Enter your virtual office' => 'Ingresa a tu Oficina Virtual',
    'Support Center' => 'Centro de Apoyo',
    'By Levels' => 'Por Niveles',
    'Direct' => 'Directos',
    'Select Country' => 'Seleccionar País',
    'Select Language' => 'Seleccionar Idioma',
    'Congratulations, you now only need to confirm your registration in your mailbox.' => 'Felicitaciones, ahora sólo falta que confirmes tu registro en tu cuenta de correo.',
    'Failed to perform your registration, please try again' => 'Ha ocurrido un error al llevar a cabo tu registro, por favor intentalo nuevamente.',
    'There was an error when registering. You are being redirected ...' => 'Hubo un error al realizar el registro. Te estamos redireccionando...',
    'Congratulations registration carried out correctly. You are being redirected ...' => 'Felicitaciones registro llevado a cabo correctamente. Te estamos redireccionando...',
    'Sponsor wrote not exist in our database.' => 'El patrocinador que escribiste no existe en nuestra base de datos.',
    'This email already exists.' => 'Este correo electronico ya existe.',
    'This user already exists.' => 'Este usuario ya existe.',
];
